...
renderCUDA(
	...
	const float* __restrict__ dL_dpixdepth_buffer,
    const float* __restrict__ dL_dpixopac,
    const float* __restrict__ dL_dpixopac_buffer,
	const int* __restrict__ blend_counts,
    ...
    float* config)
{
	...
	float accum_rec[C] = { 0 }, accum_rec_n[3] = {0}, accum_rec_d = 0, accum_rec_v = 0, accum_rec_opac = 0;
	float dL_dpixC[C], dL_dpixN[3], dL_dpixD, dL_dpixO, dL_dpixV, dL_dpixD_buffer[128], dL_dpixO_buffer[128];
	if (inside) {
		...
		for (int bui = 0; bui <128; bui ++) dL_dpixD_buffer[bui] = dL_dpixdepth_buffer[bui*H*W + pix_id];
		for (int bui = 0; bui <128; bui ++) dL_dpixO_buffer[bui] = dL_dpixopac_buffer[bui*H*W + pix_id];
	}
	...
	float D_buffer[D_buffer_size], W_buffer[D_buffer_size];
	int total_blend_count = 0;
    if (inside) {
        total_blend_count = blend_counts[pix_id];
    }
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < 1; ch++) { // grads w.r.t. depth
				...
				if (blend_count < D_buffer_size && T > 0.1){
					...
					int buffer_index = total_blend_count - blend_count - 1;

					if (buffer_index >= 0 && buffer_index < D_buffer_size) {
						atomicAdd(&(dL_ddepth[global_id]), dL_dpixdepth_buffer[buffer_index * H * W + pix_id]);
						dL_dalpha += dL_dpixopac_buffer[buffer_index * H * W + pix_id];
					}
